...
# ── Bash 版本检查 ──────────────────────────────────────────
# macOS 自带 bash 3.2，不支持 declare -A（关联数组），需要 4.0+
if ((BASH_VERSINFO[0] < 4)); then
    echo -e "\033[0;31m[ERR ]\033[0m 需要 bash 4.0+，当前版本 ${BASH_VERSION}"
    echo -e "\033[0;31m[ERR ]\033[0m macOS 用户请安装: brew install bash"
    exit 1
fi
